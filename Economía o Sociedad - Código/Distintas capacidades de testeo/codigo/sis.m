...
cap_test = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2];

economia_segun_cap_test = zeros(nsteps, length(cap_test));
contagios_segun_cap_test = zeros(nsteps, length(cap_test));
restringidos_segun_cap_test = zeros(nsteps, length(cap_test));
for k = 1:length(cap_test),
    
    cap_test_i = cap_test(k);
    ...
    while i<=nave,
        ...
        [U, tseries, adj_p]  = sis_net(N, z, nsteps, p, beta, r, i, cap_test_i);
...
economia_segun_cap_test(:, k) = suma_economia/nave;
contagios_segun_cap_test(:, k) = suma_contagios/nave;
restringidos_segun_cap_test(:, k) = suma_restringidos/nave;
