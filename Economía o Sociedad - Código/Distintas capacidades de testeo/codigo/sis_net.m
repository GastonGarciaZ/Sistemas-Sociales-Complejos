...
%%   M Zimmermann @2008
%%
function [U, tseries, adj_p] = sis_net(N, z, nsteps, p_a, beta_i, r0, n_i, cap_test_i)

   global adj p beta r CX Restricted n_tam z_tam links_0 adj_0 error cap_test
   cap_test = cap_test_i;
   r = r0;
   n_tam = N;
   z_tam =z;
...
function [pre_result] = Decission(U)
    global adj Restricted beta links_0 p r z_tam cap_test
    Dec=0;
    N = length(U);
    ...
    for i = 1:length(sel),
        if Restricted(sel(i)) == 1,
            ...
            if U_noised(sel(i)) == 0,
                ...
                contagios_corta = (sum(U.*(1-Restricted)))*(1+p*z_tam-r)-1-p*sum(adj(sel(i),:));
