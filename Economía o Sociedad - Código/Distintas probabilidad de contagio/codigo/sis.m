...
contagios_segun_p = zeros(nsteps, length(p));
restringidos_segun_p = zeros(nsteps, length(p));
for k = 1:length(p),
    ...
    while i<=nave,
        ...
        [U, tseries, adj]  = sis_net(N, z, nsteps, p_i, r, i);
...
contagios_segun_p(:, k) = suma_contagios/nave;
restringidos_segun_p(:, k) = suma_restringidos/nave;
